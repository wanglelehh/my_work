...
class WeiXinKeywordsModel extends BaseModel
{
    ...
    public function checkKey($keyword,$fromUsername){
		...
		if ($row['type'] == 'text'){
			...
			$arr['MsgType'] = 'news';
        	$rows = $this->where('pid',$row['id'])->select()->toArray();
			if (empty($rows) == false){
			...
			foreach ($rows as $key=>$row)
			{
				...
				if (strstr($row['imgurl'],'http://') == false && strstr($row['imgurl'],'https://') == false){
					$Articles[$key]['PicUrl'] = config('config.host_path').'/'.$row['imgurl'];
				}
				switch($row['bind_type']){
					case 'link':
                        if (strstr($row['data'],'http://') == false && strstr($row['data'],'https://') == false) {
