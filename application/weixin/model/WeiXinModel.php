...
class WeiXinModel extends BaseModel
{
        ...
        if (!empty($postStr)) {
            ...
            if ($postObj->MsgType == 'text') {
                ...
                if (trim($postObj->Event) == 'SCAN') {
                    ...
                    $this->keyword = empty($postObj->EventKey) ? trim($postObj->Event) : trim($postObj->EventKey);
                }
            }
        }
    }
    ...
    function getAccessToken($curlagain = false)
    {
        ...
        if (empty($access_token) || $curlagain == true) {
            ...
            Cache::set('weixin_access_token', $access_token, 7100);
        }
        return $access_token;
    }
    // ------------------------------------------
    ...
    //-- 获取微信用户信息，非关注也可以调用
    /*------------------------------------------------------ */
    function getAccWxUserInfo($openid,$access_token)
    {
        $userinfo = file_get_contents("https://api.weixin.qq.com/sns/userinfo?access_token=" . $access_token . "&openid=" . $openid . "&lang=zh_CN");
        $userinfo = json_decode($userinfo, true);
        return $userinfo;
