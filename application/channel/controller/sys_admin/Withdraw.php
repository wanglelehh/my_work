...
class Withdraw extends AdminController
{
    ...
    public function asInfo($data){
        ...
        $this->assign('withdraw_account_type', config('config.withdraw_account_type'));
        $this->assign('fee_types', config('config.fee_types'));
        return $data;
    }
    ...
    public function beforeEdit($data){
        $this->mkey = 'channelWithdrawCheck_'.$data['log_id'];
    ...
    public function afterEdit($data){
        ...
        if ($data['status'] == 1){//拒绝提现，退回帐户
            ...
            $res = $WalletModel->change($changedata, $info['user_id'], false);
            if ($res !== true) {
                Db::rollback();// 回滚事务
                return $this->error('未知错误，提现退回用户余额失败.');
            }
        }else{
            $changedata['change_desc'] = '增加累计提现';
            $changedata['change_type'] = 10;
            $changedata['by_id'] = $info['log_id'];
            $changedata['withdraw_money_total'] = $info['money'];
            $res = $WalletModel->change($changedata, $info['user_id'], false);
            if ($res !== true) {
                Db::rollback();// 回滚事务
                return $this->error('未知错误，提现退回用户余额失败.');
    ...
    public function statc(){
        ...
        $data = $this->Model->where($where)->group('account_type')->field('account_type,SUM(real_money) as realMoney,SUM(arrival_money) as totalMoney,SUM(withdraw_fee) as totalFee,count(log_id) as num')->select();
        $this->assign("data", $data);
        $totalData['num'] = 0;
    ...
    public function export(&$where)
    {
        ...
        $filename = '代理提现资料_' . date("YmdHis") . '.xls';
