...
class Withdraw  extends ApiController{
    ...
    public function getFee()
    {
        ...
        if ($maxMoney == 1){
            $money = $this->Model->getMaxMoney($this->userInfo['proxy_account']['balance_money']);
        }else{
            $money = input('money','0','float');
        }
        $data = $this->Model->checkWithdraw($this->userInfo['proxy_account']['balance_money'],$money);
    ...
    public function getLog()
    {
        ...
        $data = $this->getPageList($WithdrawLogModel, $where,'',6);
