...
class DividendModel extends BaseModel
{
    ...
    public function evalArrival($order_id = 0, $order_type = 'order', $limit_id = 0)
    {
        ...
        foreach ($rows as $row) {
            ...
            $changedata['from_uid'] = $row['buy_uid'];
            $changedata['balance_money'] = $row['dividend_amount'];
            $changedata['total_dividend'] = ($row['dividend_amount']);
            $res = $AccountModel->change($changedata, $row['dividend_uid'], false);
            if ($res !== true) {
                return false;
            }
    ...
    public function returnArrival($order_id = 0, $type = '')
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($row['status'] == $OrderModel->config['DD_DIVVIDEND']) {
                ...
                $changedata['total_dividend'] = $row['dividend_amount'] * -1;
                $res = $AccountModel->change($changedata, $row['dividend_uid'], false);
                if ($res !== true) {
                    return false;
                }
