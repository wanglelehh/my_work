...
class FgGoods extends AdminController
{
    ...
    public function getList($runData = false) {
        ...
        $viewObj->where($where)->field('fg.*,g.goods_name,g.goods_sn,g.is_spec')->order('fg.sort_order DESC, fg.fg_id DESC');
    ...
    public function checkData($row)
    {
        ...
        if ($goods['is_spec'] == 1) {
            ...
            foreach ($sku_ids as $key => $sku_id) {
                if ($fg_number[$sku_id] <= 0){
                    return $this->error('勾选的sku没有设置拼团库存.');
                }
                if ($fg_price[$sku_id] <= 0){
                    return $this->error('勾选的sku没有设置拼团价格.');
                }
                $prices[] = $fg_price[$sku_id];
            }
        } else {
            if ($fg_number <= 0){
                return $this->error('没有设置拼团库存.');
            }
            if ($fg_price <= 0){
                return $this->error('没有设置拼团价格.');
            }
            $prices[] = $fg_price * 1;
        }
