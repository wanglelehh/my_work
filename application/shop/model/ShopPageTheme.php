...
class ShopPageTheme extends BaseModel {
     ...
     * 获取模板To微信小程序
     */
	public function getToWxApp($page_id = 0){
        $mkey = 'xcx_diy_index_'.$page_id;
        $page = Cache::get($mkey);
        if (empty($page) == false){
        	return $page;
		}
		if ($page_id == 0){
            ...
            $theme = $this->where('st_id',$page_id)->find();
        }
        if (empty($theme)) return [];
        $host_path = config('config.host_path');
        $page = $theme['page'];
        $page = str_replace(['\/upload','\/a_static'],[$host_path.'\/upload',$host_path.'\/a_static'],$page);
        $page = json_decode($page,true);
        $GoodsModel = new GoodsModel();
        $roleList = (new \app\member\model\RoleModel)->getRows();
        foreach ($page['items'] as $key=>$row){
            ...
            if ($row['id'] == 'notice') {
                if ($row['params']['noticedata'] == 0) {
                    ...
                    $noticeList = $ArticleModel->where('type', 1)->where('is_show',1)->limit($row['params']['noticenum'])->select()->toArray();
                    $row['data'] = [];
                ...
                if ($row['params']['goodsdata'] > 0){
                    ...
                    }elseif ($row['params']['goodsdata'] == 5){
                        $where[] = ['is_best','=',1];
                    }

                    switch ($row['params']['goodssort']){
                        case 1://按销量
                            $sqlOrder = "sale_num DESC,virtual_sale DESC";
                            break;
                        case 2://价格降序
                    ...
                    foreach ($goodIds as $gkey=>$gid){
                        ...
                        $ginfo['limit_user_role'] = [];
                        if ($row['params']['showbuyrole'] == 1 && empty($good['limit_user_role']) == false){
                            $limit_user_role = explode(',',$good['limit_user_role']);
                            foreach ($limit_user_role as $role_id){
                                $ginfo['limit_user_role'][] = $roleList[$role_id]['name'];
                            }
                        }
                    ...
                    if ($row['params']['goodstype'] == 0) {
                        foreach ($row['data'] as $gkey => $good) {
                            if ($good['gid'] > 0) {
                                $ginfo['imgurl'] = $good['imgurl'];
                        ...
                        foreach ($row['data'] as $gkey => $good) {
                            if ($good['gid'] > 0) {
                                ...
                                $where[] = ['ig.ig_id', '=', $good['gid']];
                                $good = $IntegralGoodsModel->alias('ig')->join("shop_goods g", 'ig.goods_id=g.goods_id', 'left')->where($where)->field('ig.*,g.goods_thumb,g.goods_name,g.short_name')->find();
                                $ginfo['thumb'] = str_replace(['/upload', '/static'], [$host_path . '/upload', $host_path . '/static'], $good['goods_thumb']);
                                $ginfo['title'] = $good['goods_name'];
                                $ginfo['subtitle'] = $good['short_name'];
                                $ginfo['price'] = 0;
                ...
                $where[] = ['end_date', '>', $time];
                $limitNum = $row['params']['shownum'];
                $fgRows = $FightGroupModel->where($where)->order('sort_order DESC,fg_id DESC')->limit($limitNum+10)->select()->toArray();
                unset($row['data']);
                $i = 1;
                foreach ($fgRows as $_goods) {
                ...
                if ($row['style']['showtype'] == 1) {//划动支持
                    $snum = $row['style']['pagenum'];
                    $row['data_line'] = ceil($snum / $row['style']['rownum']);
                    $i = 0;
                    foreach ($row['data'] as $data){
                        $arr[] = $data;
                        $i++;
            ...
            }elseif ($row['id'] == 'richtext'){
                $row['params']['content'] = base64_decode($row['params']['content']);
                $row['params']['content'] = str_replace(['/upload','/static'],[$host_path.'/upload',$host_path.'/static'],  $row['params']['content']);
            }
            $page['items'][$key] = $row;
		}
