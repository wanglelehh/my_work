...
class CartModel extends BaseModel
{
    ...
    {
        parent::initialize();
        if (empty(request()->header('appsessionid')) == false){
            $this->session_id = request()->header('appsessionid');
        }else{
    ...
    public function cleanMemcache()
    {
        ...
        Cache::rm('CartInfo_' . $this->session_id . '_0' . $this->is_integral);
        Cache::rm('CartInfo_' . $this->session_id . '_1' . $this->is_integral);
    ...
    //-- 添加购物车处理
    /*------------------------------------------------------ */
    public function addToCart($goods_id, $num,  $sku_id = 0, $type = '', $prom_type = 0, $prom_id = 0)
    {
        ...
        if ($this->is_integral == 1) {
            ...
            $sub_goods = [];
            if ($sku_id > 0) {
                foreach ($goods['sub_goods'] as $sg){
                    if ($sg['sku_id'] == $sku_id){
                        $sub_goods = $sg;
                        break;
                    }
                }
                $use_integral = $sub_goods['integral'];
            } else {
        ...
        if (empty($row) == false) {// 如果购物车已经有此物品，则更新
            $update['use_integral'] = $use_integral;
            // 判断是商品能否购买或修改
            $res = $this->checkGoodsOrder($goods, $num, $sku_id, $prom_type, $prom_id, $promInfo);
            if ($res !== true) {
            ...
            if ($this->is_integral == 0) {//非积分商品执行
                $price = $GoodsModel->evalPrice($goods, $num, $sku_id,$prom_type,$promInfo);//计算需显示的商品价格
                if ($price === false) return '获取价格失败，请重试.';
                $update['sale_price'] = $price['min_price'];
                if ($goods['give_integral'] == 0) {//1:1赠送积分
                    $update['give_integral'] = $update['sale_price'];
            ...
            if ($res < 1) return '请求失败，请重试';
            $rec_id = $row['rec_id'];
        } else {// 购物车没有此物品，则插入
            $res = $this->checkGoodsOrder($goods, $num, $sku_id, $prom_type, $prom_id, $promInfo);// 判断是商品能否购买或修改
            ...
            if ($user_id > 0){
                ...
                $parent['session_id'] = $this->session_id;
            }
            $discont = 0;
            if ($this->is_integral == 0) {//非积分商品执行
                $price = $GoodsModel->evalPrice($goods, $num, $sku_id,$prom_type,$promInfo);//计算需显示的商品价格
                ...
                if ($goods['give_integral'] == 0) {//1:1赠送积分
                    $parent['give_integral'] = $parent['sale_price'];
                } elseif ($goods['give_integral'] > 0) {//赠送指定积分
            ...
            if ($sku_id > 0) {
                ...
                $parent['sku_name'] = $sub_goods['sku_name'];
    ...
    //-- 验证商品能否下单
    /*------------------------------------------------------ */
    public function checkGoodsOrder(&$goods, $num, $sku_id = 0, $prom_type = 0, $prom_id = 0, $promInfo = [])
    {
        ...
        if ($goods['is_spec'] == 1) {// 多规格商品执行
            if (empty($sku_id)) return ['code' => 0, 'msg' => '请选择商品规格后再操作'];
            $sub_goods = $goods['sub_goods'][$sku_id];
            $sku_name = $sub_goods['sku_name'];
            if ($sub_goods['is_sale'] == 0){
                return ['code' => -1, 'msg' => '商品【' . $goods['goods_name'] . ' - ' . $sku_name . '】，暂不能购买！'];
            }
            if ($sub_goods['BuyMaxNum'] < 1) return ['code' => -1, 'msg' => '商品【' . $goods['goods_name'] . ' - ' . $sku_name . '】，库存不足，暂不能购买！'];
            if ($num > $sub_goods['BuyMaxNum']) return ['code' => 0, 'msg' => '商品【' . $goods['goods_name'] . ' - ' . $sku_name . '】，只能购买' . $sub_goods['BuyMaxNum'] . '件'];
        } else {// 单规格商品执行
        ...
        if ($prom_type == 1) {
            ...
            if ($goods['is_spec'] == 1) {// 多规格商品执行
                ...
                if ($favourGoodsInfo['goods_number'] < $num) return ['code' => 0, 'msg' => '商品【' . $goods['goods_name'] . ' - ' . $sku_name . '】，活动库存，剩余：' . $favourGoodsInfo['goods_number'] . '件'];
                ...
                if ($favourGoodsInfo['goods_number'] < 1) return ['code' => -1, 'msg' => '商品【' . $goods['goods_name'] . '】，活动库存不足，暂不能购买！'];
                if ($favourGoodsInfo['goods_number'] < $num) return ['code' => 0, 'msg' => '商品【' . $goods['goods_name'] . '】，活动库存，剩余：' . $favourGoodsInfo['goods_number'] . '件'];
                if ($favourGoods['limit_num'] > 0) {
                    $ogNum = (new \app\favour\model\FavourGoodsModel)->getFavourBuyNum($promInfo);
                    if (($num + $ogNum) > $favourGoods['limit_num']) return ['code' => 0, 'msg' => '商品【' . $goods['goods_name'] . '】，当前活动档期只能购买：' . $favourGoods['limit_num'] . '件，已购买：' . $ogNum . '件'];
                }
            }
            return true;
        }
        ...
        //限制身份购买
        if (empty($goods['limit_user_role']) == false || $goods['limit_user_role'] === 0) {
    ...
    /*------------------------------------------------------ */
    public function getCartList($is_sel = 0, $no_cache = false, $recids = '', $hideSettle = true)
    {
        $user_id = $this->userInfo['user_id'] * 1;
        if ($user_id < 1) {
            $where[] = ['session_id', '=', $this->session_id];
        ...
        if (empty($recids) == false) {
            ...
            if ($user_id > 0) {
                ...
                $mkey = 'CartInfo_' . $this->session_id . '_' . $is_sel . $this->is_integral;
            }
            $data = Cache::get($mkey);
        }
        ...
        if (empty($data['allGoodsNum'])) {
            ...
            $data['allGoodsNum'] = 0;
            $data['allGoodsTotal'] = 0;
            $data['totalDiscount'] = 0;
            $data['totalGoodsPrice'] = 0;
            $data['integralTotal'] = 0;
            $rows = $this->where($where)->order('goods_id ASC,rec_id DESC')->select()->toArray();
            foreach ($rows as $key => $row) {
                ...
                $data['goodsList'][$row['goods_id'] . '_' . $row['sku_id']] = $row;
                $data['supplyerGoods'][$row['supplyer_id']][] = $row['goods_id'] . '_' . $row['sku_id'];
            }
            unset($rows);
            ...
            $data['integralTotal'] = sprintf("%.2f", $data['integralTotal']);
    ...
    public function updataGoods($rec_id, $num)
    {
        ...
        // 判断是商品能否购买或修改
        $res = $this->checkGoodsOrder($goods, $num, $cg['sku_id'], $cg['prom_type'], $cg['prom_id'], $promInfo);
        if ($res !== true) {
            return $res['msg'];
        }
        //计算需显示的商品价格
        $price = $GoodsModel->evalPrice($goods, $num, $cg['sku_id'], $cg['prom_type'], $cg['prom_id'], $promInfo);
        $arr['goods_number'] = $num;
        $arr['is_select'] = 1;
        if ($goods['is_spec'] == 1) {
            $arr['settle_price'] = $goods['sub_goods'][$cg['sku_id']]['settle_price'];
        } else {
        ...
        $arr['sale_price'] = $price['min_price'];
        if ($goods['is_spec'] == 1){
            $arr['market_price'] = $goods['sub_goods'][$cg['sku_id']]['market_price'];
        }else{
    ...
    public function delGoods($rec_id = 0)
    {
        ...
        if ($map['user_id'] == 0) $map['session_id'] = $this->session_id;
        ...
        foreach ($sf_id as $key => $val) {
            ...
            foreach ($shippingList as $code => $shipping) {
                ...
                foreach ($sf_info[$code] as $rowb) {
                    ...
                    if (empty($sfList[$setSfId][$code]) == false) {//如果已存在相关运费的模板，按初始运费对比，不论计件或计重
    ...
    public function loginUpCart($user_id)
    {
        ...
        foreach ($nRows as $row) {
            $newGoods[$row['goods_id'] . '_' . $row['sku_id']] = $row;
        }
        $oldRows = $this->where('user_id', $user_id)->select();
        if (empty($oldRows) == false) {
            $GoodsModel = new GoodsModel();
            foreach ($oldRows as $row) {
                if (empty($newGoods[$row['goods_id'] . '_' . $row['sku_id']]) == false) {//如果存在新的，删除
                    $this->where('rec_id', $row['rec_id'])->delete();
                    continue;
                }
                $goods = $GoodsModel->info($row['goods_id']);
                $upDate = array();
                $price = $GoodsModel->evalPrice($goods, $row['goods_number'], $row['sku_id']);//计算需显示的商品价格
                $upDate['sale_price'] = $price['min_price'];
        ...
        $where['session_id'] = $this->session_id;
        $where['user_id'] = 0;
        $this->where($where)->update(['user_id' => $user_id,'session_id'=>'']);
        return true;
    }
}
