...
class Goods extends AdminController
{
    ...
    public function getList($runData = false, $is_delete = 0)
    {
        ...
        $search['is_channel'] = input('is_channel',-1,'intval');
        if ($search['is_channel'] >= 0) {
            $where[] = ['is_channel', '=', $search['is_channel']];
        }
        ...
        foreach ($data['list'] as $key=>$row){
            if ($row['is_spec'] == 1){
                $row['goods_number'] = $GoodsSkuModel->where('goods_id',$row['goods_id'])->value('SUM(goods_number)');
            }
            $data['list'][$key] = $row;
        }
    ...
    public function asInfo($data)
    {
        ...
        $this->assign("skuModelList", $ModelList);//商品模型对应sku调用

        $RoleModel = new RoleModel();
        $this->assign("UsersRole", $RoleModel->getRows());//分销身份
        ...
        if ($data['goods_id'] > 0) {
            ...
            if ($data['is_spec'] == 1) {//多规格
                $GoodsSkuModel = new GoodsSkuModel();
                $skuWhere[] = ['goods_id','=',$data['goods_id']];
                $gsrows = $GoodsSkuModel->where($skuWhere)->select()->toArray();
                foreach ($gsrows as $arr) {
                    ...
                    $gsarr['sku_val'] = $arr['sku_val'];
                    $gsarr['sku_name'] = $arr['sku_name'];
                    ...
                    $gsarr['isSale'] = $arr['is_sale'];
                    $gsarr['status'] = $arr['status'];
        ...
        $this->assign('skuImgList', json_encode($skuImgList));
        $data['peers'] = json_decode($data['peers'],true);
    ...
    private function checkData($row)
    {
        ...
        if ($row['is_spec'] == 0) {//单规格
            ...
            $spec_data = input('spec_data','','htmlspecialchars_decode');
            $spec_data = json_decode($spec_data,true);
            $goods_sn = [];
            $prices = [];
            $market_price = [];
            foreach ($spec_data['trs'] as $prow) {
                ...
                if ($this->supplyer_id > 0) {
                    if (empty($prow['SettlePrice'])) {
                        return $this->error('【' . $prow['ProductSn'] . '】没有设置供货价.');
                    }
                    $prices[] = $prow['SettlePrice'];
                } else{
                    if (empty($prow['Price'])) {
                        return $this->error('【' . $prow['ProductSn'] . '】没有设置售价.');
                    }
                    $prices[] = $prow['Price'];
                }
                if ($prow['MarketPrice'] > 0){
            ...
            $row['market_price'] = empty($market_price)?0:min($market_price);
            $GoodsSkuModel = new GoodsSkuModel();
            $where[] = ['supplyer_id', '=', $this->supplyer_id];
            $where[] = ['store_id', '=', $this->store_id];
            ...
            $diffGoods = $GoodsSkuModel->where($where)->field('goods_sn,goods_id')->find();
            if (empty($diffGoods) == false){
                $is_delete = $this->Model->where('goods_id',$diffGoods['goods_id'])->value('is_delete');
                if ($is_delete == 0){
                    return $this->error('子商品货号【' . $diffGoods['goods_sn'] . '】与其它子商品货号重复.');
                }
            }
            unset($where);
        }
...
//            $row['is_alone_sale'] = isset($row['is_alone_sale']) ? 1 : 0;
            ...
            if ($row['isputaway'] == 2) {
                ...
                $row['added_time'] = strtotime($row['added_time']);
                $row['shelf_time'] = strtotime($row['shelf_time']);
                if ($row['shelf_time'] < time()){//如何下架时间小于当前时间，直接设为零
                    $row['shelf_time'] = 0;
                }
            }
        ...
        if (empty($row['video_url']) == false){
            $video_url = $row['video_url'];
            $file_path = config('config._upload_').'video/';
            if ($row['goods_id'] > 0){
                $oldVideoUrl = $this->Model->where('goods_id',$row['goods_id'])->value('video_url');
                if ($oldVideoUrl != $video_url){
        ...
        if ( settings('sys_model_channel') == 1){
            $this->ChannelGoodsModel = new \app\channel\model\ChannelGoodsModel();
            $res = $this->ChannelGoodsModel->checkData(input());
            if ($res !== true){
                return $this->error($res);
            }
        }

        $row['peers'] = json_encode($row['peers']);
        return $row;
    }
    ...
    public function afterAdd($row)
    {
        ...
        if ($row['is_spec'] == 1) {
            ...
            foreach ($spec_data['trs'] as $prow) {
                ...
                $imgwhere[] = ['store_id', '=', $this->store_id];
                $imgwhere[] = ['admin_id', '=', AUID];
                $GoodsImgsModel->where($imgwhere)->update(['goods_id'=>$row['goods_id']]);
            }
        }
        //处理优惠价格阶梯
        ...
        //代理商品
        if (settings('sys_model_channel') == 1) {
            $res = $this->ChannelGoodsModel->saveData($row['goods_id'],$row['is_channel']);
            if ($res !== true){
                Db::rollback();// 回滚事务
                return $this->error($res);
            }
        ...
        if ($row['is_spec'] == 1) {
            ...
            foreach ($spec_data['trs'] as $prow) {
                ...
                if ($this->is_supplyer == true) {//平台管理供应商处理
                    $upData['is_sale'] = $prow['isSale'] * 1;
                    $upData['shop_price'] = $prow['Price'];
                    $upData['update_time'] = $time;
                    $res = $GoodsSkuModel->where('sku_id', $prow['sku_id'] * 1)->update($upData);
                } else {
                    if (empty($prow['sku_val'])) {
                        return $this->error('操作失败:获取商品SKU值失败，请重试.');
                    }
                    ...
                    $upData['update_time'] = $time;
                    if ($prow['sku_id'] > 0) {
                        if ($prow['addStore'] > 0 || $prow['addStore'] < 0) {
                            $upData['goods_number'] = ['INC', $prow['addStore'] * 1];
                        }
                        $res = $GoodsSkuModel->where('sku_id', $prow['sku_id'])->update($upData);
                    } else {
                        $upData['sku'] = $spec_data['sku'];
                        $upData['sku_val'] = $prow['sku_val'];
            ...
            $skudelwhere[] = ['status', '=', 1];
            $skudelwhere[] = ['update_time', '<', $time];
            $count = $GoodsSkuModel->where($skudelwhere)->count();
            if ($count > 0){
                $GoodsSkuModel->where($skudelwhere)->update(['status'=>0]);//更新已取消的sku
            }
        }
        //处理优惠价格阶梯
        $volume_number = input('post.volume_number');
    ...
    public function revert()
    {
        ...
        $info = $this->Model->where('goods_id',$goods_id)->field('goods_id,goods_sn,isputaway,is_spec')->find();
        if ($info['is_spec'] == 0){
            $where[] = ['goods_sn', '=', $info['goods_sn']];
            $where[] = ['supplyer_id', '=', $this->supplyer_id];
            $where[] = ['store_id', '=', $this->store_id];
            $where[] = ['is_delete', '=', 0];
            $count = $this->Model->where($where)->count('goods_id');
            if ($count > 0) return $this->error('已存在相同的货号，不能进行恢复.');
        }else{
            $GoodsSkuModel = new GoodsSkuModel();
            $goods_sn = $GoodsSkuModel->where('goods_id',$goods_id)->column('goods_sn');
            $where[] = ['supplyer_id', '=', $this->supplyer_id];
            $where[] = ['store_id', '=', $this->store_id];
            $where[] = ['goods_sn', 'in', $goods_sn];
            ...
            if (empty($diffGoods) == false){
                $is_delete = $this->Model->where('goods_id',$diffGoods['goods_id'])->value('is_delete');
                if ($is_delete == 0){
                    return $this->error('子商品货号【' . $diffGoods['goods_sn'] . '】与其它子商品货号重复，不能进行恢复.');
                }
            }
        }
