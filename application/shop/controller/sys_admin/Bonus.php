...
class Bonus extends AdminController
{
    ...
    public function asInfo($row)
    {
        ...
        $BonusGoodsType =  $this->getDict('BonusGoodsType');
        if (settings('sys_model_shop_fightgroup') == 0){
            unset($BonusGoodsType[2]);
        }
        if (settings('sys_model_shop_favour') == 0){
            unset($BonusGoodsType[3]);
        }
        $this->assign("GoodsType", $BonusGoodsType);//指定商品类型
...
//        if ($row['type_money'] > $row['min_amount']) return $this->error('优惠券金额不能大于最小订单金额！');
    ...
    public function pubSearch()
    {
        ...
        $_list = $this->Model->where($where)->where($where)->limit(20)->select();
        $result['list'] = $_list;
        $result['code'] = 1;
