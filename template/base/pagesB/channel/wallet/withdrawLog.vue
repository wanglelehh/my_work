		...
		<swiper :current="tabCurrentIndex" class="swiper-box" duration="300" @change="changeTab">
			<swiper-item class="tab-content" v-for="(tabItem,tabIndex) in navList" :key="tabIndex">
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData">
					...
					<!-- 列表 -->
					<view v-for="(item,index) in tabItem.list" :key="index" class="item">
						...
						<view class="info">
							<view class="smll">
								<view class="w50">提现方式：{{item.account_type}}</view>
								<view class="w50">提现到帐：￥{{item.arrival_money}}</view>
							</view>
							<view class="smll mt10">
								<view class="w50">提现手续费：￥{{item.withdraw_fee}}</view>
								<view class="w50">实扣余额：￥{{item.real_money}}</view>
							</view>
						</view>
...
<script>
	...
	export default {
		...
		onLoad(options) {
			...
			//获取统计
			this.$u.post('channel/api.withdraw/getStatic').then(res => {
		...
		methods: {
			...
			loadData(source) {
				...
				this.$u.post('channel/api.withdraw/getLog', this.param).then(res => {
