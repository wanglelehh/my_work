<template>
	<view class="page-body" :class="[app.setCStyle()]">
		<view class=" ostatus mt0">
			...
			<view v-else-if="orderInfo.ostatus=='待发货'" >
				<view class="smll">
			...
			<view v-for="(item, index) in orderInfo.goodsList" :key="item.rec_id" class="flex mt20">
				...
				<view class="flex_bd ml20">
					<view class="flex">
						...
						<view class="ml10">
							<view v-if="orderInfo.order_type == 1">{{item.use_integral}}<text class="fs22">{{app.langReplace('积分')}}</text></view>
							<view v-else>
								<text class="fs22">￥</text>{{item.sale_price}}
							</view>
					...
					<view class="flex mt10">
						...
						<view class="text-right flex_bd">
							<text v-if="item.goods_number - item.after_sale_num == 0" class="fs26 b-all br30 border-color base-color ptm5 plr10" @click="app.goPage('/pages/shop/aftersale/index')">{{app.langReplace('查看售后')}}</text>
							<text v-else-if="orderInfo.isAfterSale == 1" @click="app.goPage('/pages/shop/aftersale/apply?rec_id='+item.rec_id)" class="fs26 b-all br30 border-color base-color ptm5 plr10">{{app.langReplace('申请售后')}}</text>
						</view>
					</view>
				</view>
			</view>
		...
		<view class="yt-list  bg-white">
			...
			<view v-if="setting.sys_model_shop_bonus == 1 && orderInfo.order_type != 1" class="yt-list-cell">
			...
			<view v-if="orderInfo.use_integral > 0" class="yt-list-cell">
				<text class="cell-tit clamp">{{app.langReplace('抵扣积分')}}</text>
				<text class="mr10">-</text><text class="cell-tip  ff fs32 ">{{orderInfo.use_integral}}</text>
			</view>
			<view class="yt-list-cell ">
		...
		<view class="action-box p20">
			<button v-if="orderInfo.invoice_no" class="ml10 action-btn" @click="app.goPage('shippingLog?order_id='+order_id)">{{app.langReplace('查看物流')}}</button>
			<button v-if="orderInfo.isReview == 1" class="ml10 action-btn recom" @click="app.goPage('/pages/shop/comment/my')">{{app.langReplace('立即评论')}}</button>
...
<script>
	export default {
		...
		onLoad(options) {
			...
			let title = this.app.langReplace('订单详情');
