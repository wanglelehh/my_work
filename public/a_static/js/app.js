...
$(document).on('click','.js_radio_undertake',function(){
	...
	$('.radio_undertake_'+input_name).addClass('hide');
...
$(document).on('click','.check_input_disabled',function(){
	if ($(this).is(':checked')){
        $(this).parents('.form-group-addon').find('input[type="text"]').attr("disabled",false);
	}else{
        $(this).parents('.form-group-addon').find('input[type="text"]').attr("disabled",true);
	}
})
