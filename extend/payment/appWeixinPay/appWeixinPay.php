...
class appWeixinPay
{
    ...
    {
        $notify_url = SITE_URL.'/index.php/publics/Payment/notifyUrl/pay_code/appWeixinPay';  // 接收微信支付异步通知回调地址，通知url必须为直接可访问的url，不能携带参数。
        ...
        if($inputObj['result_code'] == 'FAIL'){
            return $inputObj;
        }
        $appJson = $this->appJson($inputObj,$order['order_id']);
        return ['pay_data'=>$appJson];
