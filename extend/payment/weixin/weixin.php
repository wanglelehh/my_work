...
class weixin
{
    ...
    */
    function sendMoney($amount, $openid, $desc = '微信提现到零钱', $check_name = '')
    {
    ...
    {
        $realPath = realpath(dirname(__FILE__).'/../../../');//项目根目录地址
        $postUrl = $url;
        $curlPost = $param;
        ...
        curl_setopt($ch, CURLOPT_SSLCERT, $realPath . '/Data/cert/weixin/'.$this->pay_config['apiclient_cert_pem']); //这个是证书的位置绝对路径
        curl_setopt($ch, CURLOPT_SSLKEY, $realPath . '/Data/cert/weixin/'.$this->pay_config['apiclient_key_pem']); //这个也是证书的位置绝对路径
